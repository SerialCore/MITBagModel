...
Hadron[structure_,colorspin_,correct_]:=HadronFitting[structure,colorspin,MassVector[],BagConstant[],ZPEConstant[],correct]
...
HadronFitting[structure_,colorspin_,mass_,bagconstant_,zpe_,correction_]:=
	Module[{n=structure,CMI=colorspin,m=mass,B=bagconstant,Z=zpe,cor=correction},
		...
		M[r_]:=Sum[n[[q]]*\[Omega][m[[q]],x[[q]],r],{q,1,4}]+(4\[Pi]*r^3)/3 B-Z/r-3*CouplingConstant[r]*Sum[CMI[[p,q]]*(\[Mu][m[[p]],x[[p]],r]*\[Mu][m[[q]],x[[q]],r])/r^3 A[x[[p]],x[[q]]],{p,1,4},{q,1,4}]+cor;
