...
BindingParameterMatrix::usage="BindingParameterMatrix[] returns binding parameters matrix"
...
BindingParameterMatrix[]:=Return[{{0.304,0.287,0.098,0},{0,0.237,0.079,0},{0,0,0,0},{0,0,0,0}}]

BagConstant[]:=Return[0.145^4]

ZPEConstant[]:=Return[1.83]

Hadron[structure_,colorspin_,bindingcolor_]:=HadronFitting[structure,colorspin,bindingcolor,MassVector[],BindingParameterMatrix[],BagConstant[],ZPEConstant[]]

HadronFitting[structure_,colorspin_,bindingcolor_,mass_,bindpara_,bagconstant_,zpe_]:=
	Module[{n=structure,CMI=colorspin,bcolor=bindingcolor,m=mass,Bij=bindpara,B=bagconstant,Z=zpe},
		...
		M[r_]:=Sum[n[[q]]*\[Omega][m[[q]],x[[q]],r],{q,1,4}]+(4\[Pi]*r^3)/3 B-Z/r-3*CouplingConstant[r]*Sum[CMI[[p,q]]*(\[Mu][m[[p]],x[[p]],r]*\[Mu][m[[q]],x[[q]],r])/r^3 A[x[[p]],x[[q]]],{p,1,4},{q,1,4}]+CouplingConstant[r]*Sum[bcolor[[bp,bq]]*Bij[[bp,bq]],{bp,1,4},{bq,1,4}]/r;
